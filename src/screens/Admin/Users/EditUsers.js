...
import { editAdminProductApi, getProductDetail } from '../../../utilies/api/apiController'
...
const EditUsers = ({ navigation, route }) => {
    ...
    const getData = async () => {
        ...
        if (resp?.data?.error === false) {
            ...
            setSearchTagList(resp?.data?.data?.tags)
    ...
    const submit = async () => {
        ...
        let validate = createPostValidation({ productName, brandName, categories, price, discountPrice })
        if (validate.valid == false) {
            ...
            newProduct.append('tags', JSON.stringify(searchTagList))
    ...
    const selectTheCategorie = (index, item) => {
        ...
        let result = categoriesTemp.findIndex((category) => (
        ...
        <SafeAreaView style={Theme.TabViewCreateInsideContainer}>
            ...
            <ScrollView style={Theme.ScrollViewMargins} showsVerticalScrollIndicator={false}>
                ...
                <FormInput
                    ...
                    onChangeText={(data) => { setproductName(data); setErrortext(''); }}
                    ...
                    leftIcon={{ family: 'FontAwesome', name: 'product-hunt', color: Color.neutralGray, size: 18 }}
                    value={productName}
                    error={errortext === 'Please Enter Product Name' ? 'Please Enter Product Name' : null}
                />
                ...
                <FormInput
                    Component={Text}
                    onPress={() => { setsearchModalVisible(true); setErrortext(''); working !== 'category' && selectWorking('category') }}
                    Title={'Categories'}
                    ...
                    error={errortext === 'Please Choose atleast one category!' ? 'Please Choose atleast one category!' : null}
                />
                ...
                <FormInput
                    ...
                    onChangeText={(data) => { setSearchTags(data); }}
                    textInputContainerStyle={[Theme.InputView]}
                    style={Theme.TextInputStyle}
                    containerStyle={Theme.TextInputContainer}
                    placeholderTextColor={Color.AuthInputsPlaceholder}
                    leftIcon={{ family: 'Ionicons', name: 'pricetags-outline', color: Color.neutralGray, size: 18 }}
                    rightIcon={searchTags.length > 0 && { family: 'AntDesign', name: 'checkcircleo', color: 'green', size: 18, onPress: addSearchTag }}
                    value={searchTags}
                />
                ...
                <FormInput
                    placeholder="Price"
                    onChangeText={(data) => { setPrice(data); setErrortext('') }}
                    textInputContainerStyle={Theme.InputView}
                    style={Theme.TextInputStyle}
                    containerStyle={Theme.TextInputContainer}
                    placeholderTextColor={Color.AuthInputsPlaceholder}
                    leftIcon={{ family: 'FontAwesome', name: 'money', color: Color.neutralGray, size: 18 }}
                    keyboardType='number-pad'
                    value={price}
                    error={errortext === 'Please Choose Valid Price' ? 'Please Choose Valid Price' : errortext === 'Please Enter Price' ? 'Please Enter Price' : null}
                />

                <FormInput
                    placeholder="Discounted Price"
                    onChangeText={(data) => { setdiscountPrice(data); setErrortext('') }}
                    textInputContainerStyle={Theme.InputView}
                    style={Theme.TextInputStyle}
                    containerStyle={Theme.TextInputContainer}
                    placeholderTextColor={Color.AuthInputsPlaceholder}
                    leftIcon={{ family: 'Fontisto', name: 'shopping-sale', color: Color.neutralGray, size: 18 }}
                    keyboardType='number-pad'
                    value={discountPrice}
                    error={errortext === 'Please Enter Valid Discount Price' ? 'Please Enter Valid Discount Price' : null}
                />
            ...
            <BottomSheet width={'90%'} height={'80%'} middle modalVisible={searchModalVisible} setModalVisibility={setsearchModalVisible}>
                {
                    working != '' ? working == 'brand'
                        ?
                        <ViewBrand getData={selectTheBrand} HideBackground={false} />
                        :
                        <ViewCat getData={selectTheCategorie} HideBackground={false} /> : null
                }
